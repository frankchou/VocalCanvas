...
type IllustKind = 'hero' | 'voice' | 'script' | 'render';

// ---- Illustration renderers ----
function IllustHero(): React.JSX.Element {
  const heights = [40, 60, 90, 70, 50, 30, 70, 95, 80, 50, 30, 60, 85, 60, 40, 25];
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 28, padding: 24 }}>
      <Image src="/assets/logo-mark.svg" width={120} height={120} alt="" style={{ filter: 'drop-shadow(0 16px 36px rgba(255,90,60,0.35))' }} />
...
function IllustVoice(): React.JSX.Element {
  const voices = [
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {voices.map((v, i) => (
          <div key={i} style={{
            ...
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: v.bg, marginBottom: 8 }} />
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: 14, boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 40px', gap: 10, alignItems: 'center', fontFamily: 'var(--font-body)', fontSize: 12 }}>
          ...
          <div style={{ height: 6, background: 'var(--cream-200)', borderRadius: 3, position: 'relative' }}>
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '64%', background: 'var(--grad-sunset)', borderRadius: 3 }} />
            <div style={{ position: 'absolute', top: -5, left: '64%', transform: 'translateX(-50%)', width: 16, height: 16, borderRadius: '50%', background: '#fff', boxShadow: '0 0 0 2px var(--coral-500)' }} />
          </div>
...
function IllustScript(): React.JSX.Element {
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
      <div style={{ background: '#fff', borderRadius: 18, padding: 24, boxShadow: 'var(--shadow-md)', fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: 1.85, maxWidth: 360 }}>
        歡迎來到 VocalCanvas{' '}
        <span className="tag-delay" style={{ fontSize: 12 }}>&lt;delay 0.8s&gt;</span>
        {' '}讓你的文字{' '}
        <span className="tag-delay" style={{ fontSize: 12 }}>&lt;delay 0.5s&gt;</span>
        {' '}擁有聲音。
      </div>
    </div>
  );
...
function IllustRender(): React.JSX.Element {
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
      <div style={{ background: 'var(--ink-700)', backgroundImage: 'var(--grad-canvas)', borderRadius: 20, padding: 24, color: 'var(--cream-50)', width: '100%', maxWidth: 360, boxShadow: 'var(--shadow-xl)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2, height: 50, margin: '16px 0' }}>
          {bars.map((bar, i) => (
            <span key={i} style={{ flex: 1, height: bar.h, borderRadius: 1.5, background: bar.played ? 'var(--coral-500)' : 'rgba(255,255,255,0.18)', display: 'block' }} />
          ))}
        </div>
...
function renderIllust(kind: IllustKind): React.JSX.Element {
  if (kind === 'hero')   return <IllustHero />;
  if (kind === 'voice')  return <IllustVoice />;
  if (kind === 'script') return <IllustScript />;
  return <IllustRender />;
}
...
export default function OnboardingModal({ onClose }: OnboardingModalProps): React.JSX.Element {
  const { lang } = useLang();
    ...
    <div style={{
      ...
      <style>{`
        @media (max-width: 768px) {
          .onb-modal { grid-template-columns: 1fr !important; overflow-y: auto; }
